...
const EditSparepart = ({open,close,data,t,lang}:ModalType) => {
    ...
    const form = useForm<sparepartTableData>({
        defaultValues : {
            name : data?.name,
            code : data?.code,
            stock : data?.stock,
    ...
    const modalSubmitHandler = async (data:sparepartTableData)=>{
        ...
        if(id){
            await dispatch(editSparepart({data,id})).then( (e)=>{
            ...
            >
                <Box
                    ...
                    <div className="w-full flex justify-between items-start">
                        <h2 className="text-xl text-mainBlue capitalize font-semibold">{t('common.edit.item')}</h2>
